...
// (rollingIntervalHours, lastEventOf, averageTimeOfDay, computeStatus) were
...
// Circular mean time-of-day (as decimal hours, 0-24) across `events`, in
// `timezone`. Circular (not arithmetic) so a schedule that straddles
// midnight — e.g. bedtimes clustered around 23:45 and 00:15 — averages to
// ~00:00 instead of skewing toward noon. Returns null with no events.
export function averageTimeOfDay(events, timezone) {
  if (!events.length) return null;

  let sumX = 0;
  let sumY = 0;
  for (const e of events) {
    const hours = localHoursOfDay(new Date(e.event_time), timezone);
    const angle = (hours / 24) * 2 * Math.PI;
    sumX += Math.cos(angle);
    sumY += Math.sin(angle);
  }
  const avgAngle = Math.atan2(sumY / events.length, sumX / events.length);
  const normalized = avgAngle < 0 ? avgAngle + 2 * Math.PI : avgAngle;
...
function localHoursOfDay(date, timezone) {
