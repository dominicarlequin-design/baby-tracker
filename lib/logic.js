...
function napStatus({ events, maxSamples, fallbackHours, overdueMultiplier, now }) {
  ...
  const hoursSince = (now.getTime() - lastMs) / (1000 * 60 * 60);
  const dueAt = new Date(lastMs + avgHours * 60 * 60 * 1000);
  // Wake windows swing far more than Feed/Diaper's steadier gaps (one
  // erroneous quick nap or a longer stretch before bedtime is normal, not
  // alarming) — a late nap almost always just means she isn't tired yet,
  // not that something was missed. So nap is capped at "due-soon": it still
  // shows a due time and a gentle nudge once she's been awake a while, but
  // it never flips into the same red "overdue" alarm as a missed feed.
  const rawState = classifyState(hoursSince, avgHours, overdueMultiplier);
  const state = rawState === 'overdue' ? 'due-soon' : rawState;
...
//
// "At/after it" has a grace-minutes-wide early allowance too, not just a
// hard cutoff at the slot itself: a dose given a couple minutes before
// midnight for the 00:00 slot (the realistic way anyone actually gives a
// midnight dose) used to fall on the wrong side of `>= lastSlot` and get
// treated as never given, flipping the app to "overdue" within minutes of
// grace expiring even though the dose had just been given. Reusing
// medicine_grace_minutes as a symmetric window (early tolerance == late
// tolerance) needs no new config field and matches the existing mental
// model of "how close to the scheduled time counts as on time."
function medicineStatus({ events, config, now }) {
  ...
  const graceMs = medicine_grace_minutes * 60 * 1000;
  const graceDeadline = new Date(lastSlot.getTime() + graceMs);
  const earlyAllowance = new Date(lastSlot.getTime() - graceMs);
  const givenForSlot = medicineEvents.some(e => new Date(e.event_time).getTime() >= earlyAllowance.getTime());
