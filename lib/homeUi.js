...
export const ICON_PATHS = {
  feed: (
    <>
      {/* Bottle outline unchanged; added a neck collar ring plus a second
          fill line (round 3, "more detailed") so it reads as an actual
          marked baby bottle rather than a plain silhouette once rendered
          at the bigger sizes .icon-disc now uses. */}
  ...
  diaper: (
    <>
      {/* Round 3 redraw, take 2: a literal safety-pin-across-the-middle
          (first attempt) rendered as an unintentional rectangle at icon
          size instead of a recognizable pin. Switched to a disposable-
          diaper silhouette instead — waistband, two adhesive tab "wings"
          poking out the sides, a tapered body, and a center crease —
          which is what the button actually represents (most caregivers
          use disposable, not cloth+pin) and reads clearly at small sizes
          since every stroke is either fully horizontal/vertical or a
          single clean curve, nothing that can visually merge into a
          different shape the way the pin's circle+curve did. */}
      <path d="M7 6.5h10" />
      <path d="M7 6.5c-1.5-.3-2.7.4-2.7 1.6 0 1 .9 1.6 2 1.4" />
      <path d="M17 6.5c1.5-.3 2.7.4 2.7 1.6 0 1-.9 1.6-2 1.4" />
      <path d="M7 6.5c-.4 5.8 2.1 9.5 5 11 2.9-1.5 5.4-5.2 5-11" />
      <path d="M12 9.5v5" />
    </>
  ),
  ...
  bedtime: (
    <>
      {/* Headboard post + head, matching the reference's silhouette (a
          person's head on the pillow) rather than the generic two-pillow
          "hotel bed" glyph this replaced. */}
      <circle cx="5.3" cy="9.2" r="1.5" />
      <path d="M4 10.5V18" />
      <path d="M4 12.5h5.5a2 2 0 0 1 2 2v1.5" />
      <path d="M11.5 15.5v-1.5a2 2 0 0 1 2-2H18a2.5 2.5 0 0 1 2.5 2.5v1.5" />
      <path d="M4 15.8h16.5V18" />
      <path d="M4 18v1.3M20.5 18v1.3" />
    </>
  ),
  wake: (
    <>
...
// categories with their user-facing label + "Late by …" detail — shared by
// the Log screen's overdue banner and the server-side check-overdue route
// (app/api/check-overdue), so a category counts as overdue the same way in
// both places instead of two implementations drifting apart. `now` is
// passed in rather than read internally so both callers can use the same
// timestamp they already computed status against.
