...
// "Profile". The center "+" is a visual match for the reference's raised
// FAB, not a new quick-add flow — it links to the same log screen the
// "Today" tab does, since that screen's button grid already is this app's
// "add an entry" UI. Upcoming keeps its own page/route; it just isn't one
// of the 5 tab slots any more (its content is a fuller version of the Log
// page's own "Next Up" card, not a distinct concept in the reference).
const TABS = [
  ...
  return (
    <nav className="tab-bar">
      {TABS.map(tab => {
        if (tab.isFab) {
          return (
            <Link key={tab.key} href={tab.href} className="tab-fab" aria-label="Log an entry">
