...
// Nap must never report "overdue", no matter how far past its usual wake
// window she's been awake — wake windows swing too much (a longer stretch
// before bedtime, a skipped nap) for "late" to reliably mean something was
// missed the way it does for Feed/Diaper. It should still read "due-soon"
// once well past the average, just never escalate to overdue/red. Caught
// against real production data where overdue_multiplier=1 (average gap
// exceeded by roughly half of all normal wake windows, by definition) was
// flagging nap overdue on completely ordinary evening wake stretches.
{
  const base = new Date('2026-08-25T00:00:00Z');
  const events = [
    { id: 1, type: 'nap_start', event_time: iso(base, -9) },
    { id: 2, type: 'nap_end', event_time: iso(base, -8) },
    { id: 3, type: 'nap_start', event_time: iso(base, -6) },   // 2h wake window
    { id: 4, type: 'nap_end', event_time: iso(base, -5) },
    { id: 5, type: 'nap_start', event_time: iso(base, -3) },   // 2h wake window
    { id: 6, type: 'nap_end', event_time: iso(base, -2) },     // avg wake window: 2h
  ];
  // 20h awake since the last nap ended — 10x the usual 2h wake window, far
  // past what any overdue_multiplier would ever tolerate for Feed/Diaper.
  const status = computeStatus(events, config, new Date(iso(base, 18)));
  assert.equal(status.nap.overdue, false, 'nap must never be flagged overdue, even 10x her usual wake window');
  assert.equal(status.nap.state, 'due-soon', 'nap should cap at due-soon rather than escalate further');
}
...
// Medicine given a couple minutes BEFORE a scheduled slot (the realistic
// way a midnight dose actually gets given) must count for that slot, not
// read as "never given" just because the timestamp is a hair earlier than
// the slot itself. Real bug: 00:00 slot, dose logged at 23:58 the night
// before, grace expired at 00:15 -> app showed "Medicine overdue" minutes
// after the dose was actually given.
{
  const medConfig = { ...config, medicine_times_local: ['08:00', '16:00', '00:00'], medicine_grace_minutes: 15 };
  const midnightSlotUtc = new Date('2026-09-08T04:00:00.000Z'); // 00:00 America/New_York (EDT, UTC-4)
  const events = [
    { id: 1, type: 'medicine', event_time: new Date(midnightSlotUtc.getTime() - 2 * 60 * 1000).toISOString() }, // 23:58 local
  ];
  const status = computeStatus(events, medConfig, new Date(midnightSlotUtc.getTime() + 32 * 60 * 1000)); // 00:32 local
  assert.equal(status.medicine.overdue, false, 'a dose given 2 minutes early must satisfy that slot, not read as missed');
}

// The same early allowance must not stretch far enough to let a dose meant
// for an earlier slot also cover a later one — a dose from the 16:00 slot
// logged on time should NOT satisfy the 00:00 slot just because "early
// allowance" exists; the 00:00 dose still has to actually happen.
{
  const medConfig = { ...config, medicine_times_local: ['08:00', '16:00', '00:00'], medicine_grace_minutes: 15 };
  const midnightSlotUtc = new Date('2026-09-08T04:00:00.000Z');
  const events = [
    { id: 1, type: 'medicine', event_time: new Date(midnightSlotUtc.getTime() - 8 * 60 * 60 * 1000).toISOString() }, // the 16:00 dose, given on time
  ];
  const status = computeStatus(events, medConfig, new Date(midnightSlotUtc.getTime() + 32 * 60 * 1000)); // 00:32 local, past grace
  assert.equal(status.medicine.overdue, true, 'an earlier slot\'s dose must not satisfy a later slot it was never intended for');
}
